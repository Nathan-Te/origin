...
int main(int argc, char *argv[])
{
    ...
    Mix_PlayMusic(musique, -1);
      ...
      for(i=0;i<196;i++){
        ...
        if (bullet_shot == true){
          for (int i = 0; i < 19 ; i++){
            SDL_Log("la bullet : %d live_bullet : %d", i, live_bullet[i]);
        ...
        if(shooting == true && look == 0){
          ...
          if (animation_shoot == 0){
            if (compteur_animation == 0 && live_bullet[nb_bullet] == -1){
              bullet_shot = true;
              dir_bullet[nb_bullet] = 0;
              live_bullet[nb_bullet] = 1;
              //Mix_PlayChannel(1, sound_bullet, 0);
              pTexture_bullet[nb_bullet] = initialiser_image("bullet_top.png",dst1.x+25,dst1.y-8,5,8,&rect_NULL,&dst_bullet[nb_bullet],pRenderer);
          ...
          if (animation_shoot == 0){
            if (compteur_animation == 0 && live_bullet[nb_bullet] == -1){
              ...
              live_bullet[nb_bullet] = 1;
              //Mix_PlayChannel(1, sound_bullet, 0);
              pTexture_bullet[nb_bullet] = initialiser_image("bullet_down.png",dst1.x+8,dst1.y+20,5,8,&rect_NULL,&dst_bullet[nb_bullet],pRenderer);
              //pTextureImage_bullet = initialiser_image("bullet_left.png",dst1.x-15,dst1.y+21,8,5,&src_bullet1,&dst_bullet1,pRenderer);
              nb_bullet++;
            }
            SDL_Log("animation 0");
