...
int main(int argc, char *argv[])
{
    ...
    projectile_t projectile_tab[20];
    ...
    SDL_SetRenderDrawColor(pRenderer, 255, 255, 255, 255);
      ...
      for(i=0;i<196;i++){
        ...
        if (bullet_shot == true){
          for (int i = 0; i < nb_bullet ; i++){
            SDL_Log("Projectile nb : %d De direction : %d", i, projectile_tab[i].direction);
            if (/* dst_bullet[i].x */ projectile_tab[i].dst_bullet.x >= -10){
              projectile_tab[i].dst_bullet.x -= 1;
              //dst_bullet[i].x -= 1;
            }
          }
          //dst_bullet1.x -= 1;
        ...
        if (shooting == true && look == 2){
          ...
          if (animation_shoot == 0){
            ...
            if (compteur_animation == 0){
              bullet_shot = true;
              projectile_tab[nb_bullet].pTexture_bullet = initialiser_image("bullet_left.png",dst1.x-10,dst1.y+21,8,5,&rect_NULL,&dst_bullet[nb_bullet],pRenderer);
              projectile_tab[nb_bullet].direction = 2;
      ...
      for(i=0;i<nb_bullet;i++){
        afficher_image(pRenderer, projectile_tab[i].pTexture_bullet, rect_NULL, projectile_tab[i].dst_bullet);
